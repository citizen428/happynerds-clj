(defproject
  ...
  [[prone "0.8.0"]
   ...
   [ring-server "0.3.1"]]
  :repl-options
  {:init-ns happynerds-clj.repl}
  :jvm-opts
  ["-server"]
  :plugins
  [[lein-ring "0.9.0"] [lein-environ "1.0.0"] [lein-ancient "0.5.5"]]
  :ring
  {:handler happynerds-clj.handler/app,
   :init happynerds-clj.handler/init,
   :destroy happynerds-clj.handler/destroy,
   :uberwar-name "happynerds-clj.war"}
  :profiles
